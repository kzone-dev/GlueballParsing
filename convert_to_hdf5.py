...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert glueball operator data from txt to HDF5 format."
    )
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Path to the input txt file."
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Path to the output HDF5 file."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="String to identify the operator type in the txt files (e.g., 'A1pp' or 'A1mp').",
    )
...
if __name__ == "__main__":
    ...
    output_hdf5 = os.path.join(
        output_data_dir, f"{ensemble}_{rpc_str}_glueball_operators.hdf5"
    )
